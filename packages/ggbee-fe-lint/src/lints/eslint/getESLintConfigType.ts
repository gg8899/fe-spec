 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-ggbee/index
 * @returns eslint-config-ggbee/react
 * @returns eslint-config-ggbee/typescript/index
 * @returns eslint-config-ggbee/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'eslint-config-ggbee/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
